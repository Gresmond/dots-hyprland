...
MouseArea {
    ...
    // Caps Lock State Monitor
    property bool capsLockOn: false

    Process {
        id: capslockWatcher
        command: ["sh", "-c", "grep -q '^[1-9]' /sys/class/leds/input*::capslock/brightness 2>/dev/null && echo true || (hyprctl devices -j 2>/dev/null | grep -q '\"capsLock\": true' && echo true || echo false)"]
        stdout: SplitParser {
            onRead: data => {
                root.capsLockOn = (data.trim() === "true");
            }
        }
    }
    ...
    Timer {
        ...
        running: true
        repeat: true
        onTriggered: capslockWatcher.running = true
    }
        ...
        ToolbarTextField {
            ...
            Loader {
                ...
                anchors {
                    ...
                    rightMargin: root.capsLockOn ? (passwordBox.padding + 20) : passwordBox.padding
                }
            ...
            // Caps Lock Indicator inside password field
            MaterialSymbol {
                anchors {
                    right: parent.right
                    rightMargin: 10
