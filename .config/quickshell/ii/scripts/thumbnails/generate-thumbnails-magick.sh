...
generate_thumbnail() {
    ...
    # Handle videos, gifs, and static images
    case "${abs_path,,}" in
        *.mp4|*.webm|*.mkv|*.avi|*.mov)
            ffmpegthumbnailer -i "$abs_path" -o "$out" -s "$THUMBNAIL_SIZE" -q 8 2>/dev/null || \
            ffmpeg -y -ss 00:00:01 -i "$abs_path" -vframes 1 -vf "scale=${THUMBNAIL_SIZE}:-1" "$out" 2>/dev/null
            ;;
        *.gif)
            magick "${abs_path}[0]" -resize "${THUMBNAIL_SIZE}x${THUMBNAIL_SIZE}" "$out" 2>/dev/null
            ;;
        *)
